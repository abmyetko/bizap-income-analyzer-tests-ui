...
@DisplayName("Secondary job defaults test for Fha Hourly income")
@EnableIfToggled(propertyName = SECONDARY_JOBS_TOGGLE)
@Execution(CONCURRENT)
@ResourceLock(value = "SecondaryHourlyFhaDefaultsTest")
class SecondaryHourlyFhaDefaultsTest extends TestBaseRest {

    private final IAFolderBuilder folderBuilderSecHrFhaDef = createFolderBuilder("IARSecFHDef");

    @BeforeAll
    void uploadLoanDocAndIncomeDocuments() {
        folderBuilderSecHrFhaDef.setMortgageAppliedFor(MortgageType.FHA)
                .generateSecondaryJobsLoanDocument()
                .restBuild();
        dataUpload = createUploadObject(folderBuilderSecHrFhaDef);

        RestGetLoanData.getApplicationData(folderBuilderSecHrFhaDef.getFolderId());
    }
    ...
    void checkExpectedCalculations(SummaryDocumentType ytdDefaultDoc, AnnualSummaryIncomeType ytdAnnualSummaryIncomeType, IncomeAvg haSelection) {
        RestGetResponseApplicant applicant = RestGetLoanData.getApplicationData(folderBuilderSecHrFhaDef.getFolderId()).getApplicant(folderBuilderSecHrFhaDef.getBorrowerFullName());
        RestPartIncome applicantCurrentJobYtdIncomes = applicant.getIncome(folderBuilderSecHrFhaDef.getBorrowerSecondEmployment()).getBasePay().getIncome(YEAR_TO_DATE);
    ...
    void generateZeroIncomeDocs() {
        dataUpload.clearDocuments()
                .addDocument(dataUpload.createCustomPaystub(
                        folderBuilderSecHrFhaDef.getBorrowerFullName(),
                        folderBuilderSecHrFhaDef.getBorrowerSSN(),
                        folderBuilderSecHrFhaDef.getBorrowerCollaboratorId(),
                        folderBuilderSecHrFhaDef.getBorrowerSecondEmployment(),
                        "01/01/" + YEAR_TO_DATE,
                        "01/31/" + YEAR_TO_DATE,

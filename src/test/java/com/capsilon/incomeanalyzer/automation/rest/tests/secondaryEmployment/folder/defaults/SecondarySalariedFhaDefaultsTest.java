...
@DisplayName("Secondary job defaults test for Fha Salaried income")
@EnableIfToggled(propertyName = SECONDARY_JOBS_TOGGLE)
@Execution(CONCURRENT)
@ResourceLock(value = "SecondarySalariedFhaDefaultsTest")
class SecondarySalariedFhaDefaultsTest extends TestBaseRest {

    private final IAFolderBuilder folderBuilderSecSalFhaDef = createFolderBuilder("IARSecFSDef");

    @BeforeAll
    void uploadLoanDocAndIncomeDocuments() {
        folderBuilderSecSalFhaDef.setMortgageAppliedFor(MortgageType.FHA)
                .generateSecondaryJobsLoanDocument()
                .restBuild();
        dataUpload = createUploadObject(folderBuilderSecSalFhaDef);

        RestGetLoanData.getApplicationData(folderBuilderSecSalFhaDef.getFolderId());
    }
    ...
    void checkExpectedCalculations(SummaryDocumentType ytdDefaultDoc, AnnualSummaryIncomeType ytdAnnualSummaryIncomeType, IncomeAvg haSelection) {
        RestGetResponseApplicant applicant = RestGetLoanData.getApplicationData(folderBuilderSecSalFhaDef.getFolderId()).getApplicant(folderBuilderSecSalFhaDef.getBorrowerFullName());
        RestPartIncome applicantCurrentJobYtdIncomes = applicant.getIncome(folderBuilderSecSalFhaDef.getBorrowerSecondEmployment()).getBasePay().getIncome(YEAR_TO_DATE);
    ...
    void generateZeroIncomeDocs() {
        dataUpload.clearDocuments()
                .addDocument(dataUpload.createCustomPaystub(
                        folderBuilderSecSalFhaDef.getBorrowerFullName(),
                        folderBuilderSecSalFhaDef.getBorrowerSSN(),
                        folderBuilderSecSalFhaDef.getBorrowerCollaboratorId(),
                        folderBuilderSecSalFhaDef.getBorrowerSecondEmployment(),
                        "01/01/" + YEAR_TO_DATE,
                        "01/31/" + YEAR_TO_DATE,
